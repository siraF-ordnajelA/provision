...
select LEFT ([Técnico], 100),
	...
	LEFT ([Desviado], 3),
	LEFT ([Ubicacion], 50),
	case when [Enrutado automático a fecha] = '' then null
			when [Enrutado automático a fecha] = '-' then null else '20' + RIGHT (REPLACE([Enrutado automático a fecha],'/',1), 2) + '-' + LEFT (REPLACE([Enrutado automático a fecha],'/',1), 2) + '-' + SUBSTRING (REPLACE([Enrutado automático a fecha],'/',1), 4, 2) end as [Enrutado auto],
